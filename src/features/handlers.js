import { Signs, isAnswer, MAX_DISPLAY_LENGTH } from "./constants.js";
...
export function handleButtonClick() {
  ...
  if (display.textContent.length >= MAX_DISPLAY_LENGTH) {
    confirm("Limit of expression length, solve it? If not, the field will be cleared.")
      ? handleEquals()
...
function shouldResetDisplay() {
  return display.textContent === "0" || display.textContent === "Error" || isAnswer;
}
