export function changeSign() {
  ...
  if (/^-?\d+\.?\d*%?$/.test(currentValue)) {
    if (currentValue.includes("%")) {
      ...
      display.textContent = num.startsWith("-") ? num.substring(1) + "%" : "-" + num + "%";
    } else {
  ...
  if (operatorPercentMatch) {
    const [fullMatch, operator, percent] = operatorPercentMatch;
    const newOperator = operator === "+" ? "-" : "+";
    display.textContent = currentValue.replace(operatorPercentRegex, `${newOperator}${percent}`);
    return;
  }
  ...
  if (operatorNumberMatch) {
    const [fullMatch, operator, number] = operatorNumberMatch;
    const newOperator = operator === "+" ? "-" : "+";
    display.textContent = currentValue.replace(operatorNumberRegex, `${newOperator}${number}`);
    return;
  }
}
